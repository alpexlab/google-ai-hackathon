...
import { getSkinReport } from '@/services/backend';
import { _SKIN_REPORT } from '@/types';
...
const Skin = () => {
  ...
  const [report, setReport] = useState<_SKIN_REPORT>();
  const [params] = useSearchParams();
  const patientId = params.get('patient') as string;

  useEffect(() => {
    async function fetchReport() {
      const response = await getSkinReport(id as string);
      ...
      if (response.report.status === 'COMPLETE') {
        ...
        if (segmented_image && !segmented_image.startsWith('http')) {
